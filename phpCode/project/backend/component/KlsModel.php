...
class KlsModel
{
	...
	public static function excludeHeaderRows(array $records) {
		if(count( $records ))
		{
			$lastHeaderRowIndex = self::DEFAULT_HEADER_SIZE;
			$headerRowRange = range( 0, $lastHeaderRowIndex - 1);
			$headerRowKeys = array_flip($headerRowRange);
	...
	public static function normalizeColumnsNumber(array &$data) {
		if( count( $data ) )
		{
			...
			if( count( $sample ) > $attributesCount )
			{
				...
				{
					array_walk( $data, function ( &$item, $i, $count ) {
	...
	public function attributeLabels() {
		return [
			  'kls_code'           => 'Код раздела',
			  'kls_names'          => 'Краткое наименование раздела',
			  'kls_namef'          => 'Полное наименование раздела',
			  'kls_rubrika'        => 'Рубрика раздела'
		];
	}
	...
	protected function prepareDataForRequest(array $data = [], $force = false)
	{
		...
		return AH::merge( $attributesData, $data );
	}
