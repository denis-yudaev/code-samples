...
abstract class odsStyle
{
	 ...
	 * @return \DOMElement
	 */
	protected function getContent(/** @noinspection PhpUnusedParameterInspection */ ods $ods, \DOMDocument $dom) {
	...
	public function __clone() {
		$this->name = 'clone-' . $this->name . '-' . mt_rand(0,99999999999);
	}
	...
	/**
	 * @param odsStyle|string $name экземпляр объекта стиля, или имя соответствующего класса
	 *
	 * @return odsStyle $this чтобы можно было продолжать вешать методы по-цепочке... ;)
	 */
	public function setParentStyle($name){
		$this->parentStyleName = is_object($name) ? $name->getName() : $name;

		return $this;
	}

	/** Альяс метода setParentStyle
	 *  @see odsStyle::setParentStyle()
	 */
	public function setParentStyleName(/** @noinspection PhpDocSignatureInspection */ $name){
		return $this->setParentStyle( $name );
	}
	...
	protected function randString(){
		return md5( time() . mt_rand() . $this->getType() );
	}
...
class odsStyleNumber extends odsStyle
{
	protected $decimalPlaces;      // integer
	protected $minIntegerDigits;             // integer
	protected $grouping;         // "true", "false"
	protected $language;         // "true", "false"
	protected $country;         // "true", "false"
	...
	public function __construct($name = null) {
		...
		$this->name   = $name;
		$this->dataStyleName   = false;
		$this->language = 'ru'; //  языковые настройки
		$this->country = 'RU';  //  страна, локальный формат которой стоит использовать
	...
	public function getContent(ods $ods, \DOMDocument $dom) {
		...
		if($this->language) {
			$style->setAttribute('number:language', $this->language );
		}
		if($this->country) {
			$style->setAttribute('number:country', $this->country );
		}
		...
		if($this->decimalPlaces){
			...
			$number->setAttribute('loext:min-decimal-places', $this->decimalPlaces );
		}
...
class odsStyleTableRow extends odsStyle {
	...
	public function __construct($name = null) {
		...
		$this->minRowHeight =       false;
		$this->breakBefore         = 'auto';
		$this->useOptimalRowHeight = 'true';
		$this->styleKeepTogether = 'false';
		$this->foKeepTogether  = 'always';
	}
...
class odsStyleTableCell extends odsStyle {
	...
	private $wrapOption;          // opt: 'wrap', 'no-wrap'
...
class odsStyleMoneyRUB extends odsStyleMoney {

	public function __construct() {
		$this->name='NCur-RUB-P0';
	}

	public function getContent(ods $ods, \DOMDocument $dom) {
		$number_currency_style = $dom->createElement('number:currency-style');
		$number_currency_style->setAttribute("style:name", "NCur-RUB-P0");
		...
		$number_currency_symbol->setAttribute("number:language", "ru");
		$number_currency_symbol->setAttribute("number:country", "RU");
		$number_number->setAttribute("number:grouping", "true");
	...
	public function getType() {
		return 'odsStyleMoneyRUB';
	}
}

class odsStyleMoneyRUBNeg extends odsStyleMoney {
	...
	public function getContent(ods $ods, \DOMDocument $dom) {
		...
		$number_currency_style = $dom->createElement('number:currency-style');
		$number_currency_style->setAttribute("style:name", "NCur-RUB");
		...
		$style_map->setAttribute("style:apply-style-name", "NCur-RUB-P0");
