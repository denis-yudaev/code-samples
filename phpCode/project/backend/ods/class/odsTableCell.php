...
class odsTableCellEmpty extends odsTableCell
{
	...
	public function __construct($odsStyleTableCell = null) {
...
class odsTableCellStringHttp extends odsTableCell
{
	...
	public function __construct($value, $odsStyleTableCell = null) {
		$this->value = $value;
		$this->styleName = $odsStyleTableCell;
	}
...
class odsTableCellCurrency extends odsTableCell
{
	...
	public function getContent(ods $ods, \DOMDocument $dom) {
		switch ($this->currency) {
			case 'RUB':
				$ods->addTmpStyles(new odsStyleMoneyRUB());
				$ods->addTmpStyles(new odsStyleMoneyRUBNeg());
				break;
			case 'EUR':
...
class odsTableCellDate extends odsTableCell
{
	...
	public function __construct(
		...
		$format = "MMDDYYYY",
		$language = null,
		odsStyleGraphic $odsStyleCellDate = null
	) {
		$this->date = $date;
		$this->format = $format;
...
class odsTableCellComplex extends odsTableCell
{
	...
	/**
	 * @param mixed                  $value
	 * @param OdsStyleTableCell|null $odsStyleTableCell
	 */
	public function __construct($value = null, $odsStyleTableCell = null) {
		$this->styleName = $odsStyleTableCell;
	...
	public function getContent(ods $ods, \DOMDocument $dom) {
		$element = odsTableCell::getContent($ods, $dom);
		...
		if (!empty($childList)) {
			foreach ($childList as $child) {
				if (is_string($child)) {
					...
					$node = is_object($child) ? $child->getContent($ods, $dom) : $dom->createTextNode($child);
				}
